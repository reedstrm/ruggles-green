...
public class ParallelModuleMigrator implements Runnable {
    ...
    private final IdWrapper moduleId;
    private final VersionWrapper currentVersion;
    private final boolean isMigration;

    private boolean success = false;
    private ModuleWrapper responseModuleWrapper;
     ...
     * 
     * @param moduleId ModuleId on cnx.org.
     * @param currentVersion Current Version.
     */
    public ParallelModuleMigrator(CnxClient cnxClient, String moduleLocation,
            IdWrapper moduleId, VersionWrapper currentVersion, boolean isMigration) {
        this.cnxClient = cnxClient;
        this.moduleLocation = moduleLocation;
        this.moduleId = Preconditions.checkNotNull(moduleId);
    ...
    public ModuleWrapper migrateModuleVersion() {
        ...
        for (int i = 0; i < 10; i++) {
            try {
                ...
                try {
                    moduleVersionWrapper = cnxClient.getModuleVersion(moduleId, currentVersion);
                } catch (Throwable e) {
                    e.printStackTrace();
                }

                URI editUri = null;
                if (moduleVersionWrapper != null) {
                    editUri = moduleVersionWrapper.getEditUri();
                } else {
                    ...
                    editUri = moduleWrapper.getEditUri();
                }

                responseModuleWrapper =
                        cnxClient.createModuleVersion(editUri, cnxml, resourceMappingXml);
                success = true;

                logger.info("Successfully uploaded : "
                        + moduleId.getId()
                        + " to : " + responseModuleWrapper.getEditUri());
                return responseModuleWrapper;
            } catch (Exception e) {
                e.printStackTrace();
