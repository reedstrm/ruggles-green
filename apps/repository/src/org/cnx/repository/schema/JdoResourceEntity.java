...
public class JdoResourceEntity {
	...
	public enum State {
		/**
		 * Initial state upon instantiation. Entity is never persisted in this
		 * state.
		 */
		IDLE,
	...
	/**
	 * The key of this resource in the data store. Assigned automatically by the
	 * data store first time the entity is persisted. Unique only within the key
	 * of this entity type. The externally exposed resource id is derived from
	 * this key.
	 */
	@PrimaryKey
	...
	/**
	 * The blob key of this resource. Exists in states where hasBlobKey() is
	 * true, null otherwise.
	 */
	@Persistent
	...
	/**
	 * Get the id of this resource. The id is unique only among resource
	 * entities.
	 * 
	 * Entity id is populated first time the entity is persisted. Until then
	...
	public void idleToPendingTransition() {
		Assertions.check(state == State.IDLE, "Encountered %s", state);
		state = State.PENDING_UPLOAD;
	}

	public void pendingToUploadedTransition(BlobKey newBlobKey) {
		Assertions
				.check(state == State.PENDING_UPLOAD, "Encountered %s", state);

		blobKey = Assertions.checkNotNull(newBlobKey, "Null blob key");
	...
	/**
	 * Convert a resource entity id to externally visible string encoded id.
	 * This is the inverse operation of stringToResouceId();
	 * 
	 * @param resourceKey
	 *            a valid resource entity key.
	...
	/**
	 * Convert a resource entity id to externally visible resource id. The is
	 * the inverse operation of resourceIdToString();
	 * 
	 * @param idString
	 *            an externally visible resource id generated by keyToString();
